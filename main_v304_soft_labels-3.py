...
formula. One deliberate exception, added after review, is no longer
optional -- see immediately below.
  ...
  - CFG.STRICT_CALIBRATION_HOLDOUT now defaults to True (changed from
    False). Under the old split, the calibration window (2017-2019) used
    to fit temperature scaling was a *subset* of the training window
    (train = all dates before the test start), so calibration was fit on
    in-sample model outputs. That never affected the headline detection
    metrics (recall / precision / F1 / ROC-AUC / PR-AUC), which are all
    computed on the genuinely held-out 2020+ test set -- but it could
    optimistically bias the calibration-quality numbers (ECE / Brier /
    log-loss), since a model's outputs on data it trained on tend to be
    more extreme than on truly unseen data.

    IMPORTANT, read before re-running: fixing this moves the training
    cutoff from 2020 back to 2017, which means the model now trains on
    about three fewer years of data. That changes the trained model
    itself, not just the calibration step -- every number in the report,
    including ROC-AUC / PR-AUC / recall / precision, can shift somewhat
    on the next run, not only ECE / Brier / log-loss. Numbers already
    published anywhere (a repo description, a paper draft, a slide) that
    came from the old default will not necessarily match a re-run under
    this new default. Re-run the full pipeline and update anything you've
    already published with the new numbers; do not assume only the
    calibration section changed. The old behavior is still available by
    passing STRICT_CALIBRATION_HOLDOUT=False if you specifically need to
    reproduce the original published V30.4 numbers.
...
class CFG:
    ...
    # -- Methodology switch -- True gives a strictly disjoint train/
    # calibration/test split (fixes the in-sample calibration bias, see the
    # module docstring above). Set to False only to reproduce the original
    # published V30.4 numbers exactly; that run is no longer the default
    # because those numbers were shown to be optimistically biased on the
    # three calibration metrics. --
    STRICT_CALIBRATION_HOLDOUT = True
...
def split_data(dates: pd.DatetimeIndex, X: np.ndarray, y_hard: np.ndarray,
    ...
    By default (cfg.STRICT_CALIBRATION_HOLDOUT = True) train stops at the
    calibration window's start (2017-01-01), giving a fully disjoint
    train / calibration / test split. Set the flag to False only to
    reproduce the original published V30.4 split, where train ran through
    2019-12-31 and therefore included the calibration slice -- see the
    module docstring for exactly what that changes and why it is no
    longer the default.
    """
...
def main() -> None:
    ...
    # ---- stateful sequential inference: calibration set ----
    logger.info("running stateful ensemble inference (calibration set)")
    X_calib_t = torch.tensor(split["X_calib"], dtype=torch.float32).to(CFG.DEVICE)
    psi_calib_runs = []
    for model in models:
        model.eval()
        with torch.no_grad():
            state = torch.zeros((1, CFG.LATENT_DIM), device=CFG.DEVICE)
            buffer = []
            for t in range(X_calib_t.shape[0]):
                psi_t, _, state = model(X_calib_t[t:t + 1], s_initial=state)
                buffer.append(psi_t.cpu().item())
            psi_calib_runs.append(np.array(buffer))
    psi_calib_ensemble = np.mean(psi_calib_runs, axis=0)

    t_calib = get_temperature_scaler(psi_calib_ensemble, split["y_calib_hard"])
    logger.info("T_calib = %.3f", t_calib)

    # ---- stateful sequential inference: test set ----
    logger.info("running stateful ensemble inference (test set)")
    X_test_t = torch.tensor(split["X_test"], dtype=torch.float32).to(CFG.DEVICE)
    psi_runs, gamma_runs, temp_runs, alpha_runs = [], [], [], []
    for model in models:
        model.eval()
        with torch.no_grad():
            state = torch.zeros((1, CFG.LATENT_DIM), device=CFG.DEVICE)
            bp, bg, bt, ba = [], [], [], []
            for t in range(X_test_t.shape[0]):
                psi_t, _, state = model(X_test_t[t:t + 1], s_initial=state)
            ...
            psi_runs.append(np.array(bp))
            gamma_runs.append(np.array(bg))
            temp_runs.append(np.array(bt))
            alpha_runs.append(np.array(ba))

    psi_ensemble = np.mean(psi_runs, axis=0)
    gamma_ensemble = np.mean(gamma_runs, axis=0)
    temp_ensemble = np.mean(temp_runs, axis=0)
    alpha_ensemble = np.mean(alpha_runs, axis=0)

...
reporting. It does not change any model, hyperparameter, or formula from
the version above -- every number already validated for V30.4 reproduces
exactly under the default configuration. Specifically it:
  ...
  - Adds a documented, OFF-by-default CFG.STRICT_CALIBRATION_HOLDOUT flag.
    Under the current split, the calibration window (2017-2019) used to fit
    temperature scaling is a *subset* of the training window (train = all
    dates before the test start), so calibration is fit on in-sample model
    outputs. This does not affect the headline detection metrics (recall /
    precision / F1 / ROC-AUC / PR-AUC), which are all computed on the
    genuinely held-out 2020+ test set -- but it can optimistically bias the
    calibration-quality numbers (ECE / Brier / log-loss), since a model's
    outputs on data it trained on tend to be more extreme than on truly
    unseen data. Flip the flag for a fully disjoint 3-way split before
    quoting those three numbers in a paper; it changes what the model
    trains on, so treat it as a methodology check to re-run and report
    separately, not a silent fix.
...
class CFG:
    ...
    # -- Methodology switch (new in this refactor, default preserves the
    # exact V30.4 split/results) --
...
def split_data(dates: pd.DatetimeIndex, X: np.ndarray, y_hard: np.ndarray,
    ...
    By default (cfg.STRICT_CALIBRATION_HOLDOUT = False) this reproduces the
    exact split behind every V30.x result so far: train = everything before
    the test window, which includes the 2017-2019 calibration slice. See
    the module docstring for what that does and does not affect.
    """

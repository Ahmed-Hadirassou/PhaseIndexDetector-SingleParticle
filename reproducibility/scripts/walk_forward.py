...
LEAD_TIME_LOOKBACK_DAYS = 90  # trading days searched before official crisis onset


def _lead_time_days(p_calib: np.ndarray, test_index: pd.DatetimeIndex, y_test_hard: np.ndarray,
                     tau: float, crisis_start: str,
                     lookback_days: int = LEAD_TIME_LOOKBACK_DAYS):
    """Calendar days between the first day p_calib >= tau and the crisis's
    official onset, searched over the LAST `lookback_days` TRADING days
    strictly before onset that are not themselves inside another labeled
    crisis (so a neighboring, separate crisis's detection can't be
    misattributed as this one's lead time). Returns None if p_calib never
    crosses tau in that window -- i.e. no early warning captured for this
    crisis, not zero and not a negative number standing in for "didn't
    happen"."""
    s = pd.Timestamp(crisis_start)
    pre_mask = (test_index < s) & (y_test_hard == 0)
    pre_idx = np.where(pre_mask)[0]
    if len(pre_idx) == 0:
        return None
    pre_idx = pre_idx[-lookback_days:] if len(pre_idx) > lookback_days else pre_idx
    crossed = pre_idx[p_calib[pre_idx] >= tau]
    if len(crossed) == 0:
        return None
    first_cross_pos = int(crossed.min())
    return int((s - test_index[first_cross_pos]).days)
...
def evaluate_fold(fold_id, train_end, test_start, test_end,
    ...
    # In-sample vs out-of-sample AUC gap: reuses the SAME already-trained
    # ensemble, one extra inference pass over the full training window --
    # no retraining, negligible cost. A large gap (train >> test) is the
    # cheapest, most classic overfitting signal there is.
    y_train_hard = y_hard[train_mask]
    train_auc = None
    if 0 < y_train_hard.sum() < len(y_train_hard):
        psi_train_full = mvsl.run_stateful_inference(models, X_train, mvsl.CFG.DEVICE, mvsl.CFG.LATENT_DIM)
        train_auc = float(roc_auc_score(y_train_hard, psi_train_full))
    ...
    result = {
        ...
        "t_calib": float(t_calib), "train_auc": train_auc,
        "crises": [],
    }
    ...
    if n_crisis == 0 or n_crisis == n_test:
        ...
        result.update({"roc_auc": None, "pr_auc": None, "cohens_d": None,
                        "recall": None, "precision": None, "f1": None, "ece": None,
                        "auc_gap": None})
    ...
    ece = em.expected_calibration_error(y_test_hard, p_calib, n_bins=10)
    test_auc = float(roc_auc_score(y_test_hard, psi_test))
    ...
    for crisis in crises_in_window(test_index):
        ...
        if cmask.sum() > 0:
            ...
            lead_time = _lead_time_days(p_calib, test_index, y_test_hard, tau, crisis["window"][0])
            result["crises"].append({"name": crisis["name"], "n_days": int(cmask.sum()),
                                      "recall_calibrated": c_recall, "lead_time_days": lead_time})

    gap_str = f"{auc_gap:+.4f}" if auc_gap is not None else "n/a"
    train_auc_str = f"{train_auc:.4f}" if train_auc is not None else "n/a"
    print(f"    ROC-AUC={result['roc_auc']:.4f}  PR-AUC={result['pr_auc']:.4f}  "
          f"F1={f1:.4f}  ECE={ece:.4f}  d={result['cohens_d']:.2f}  "
          f"recall={recall:.3f}  precision={precision:.3f}")
    print(f"    train_AUC={train_auc_str}  gap(train-test)={gap_str}")
    for c in result["crises"]:
        lt = f"{c['lead_time_days']}d early" if c["lead_time_days"] is not None else "no early crossing"
        print(f"      {c['name']:32s} recall={c['recall_calibrated']*100:5.1f}%  "
              f"({c['n_days']}d)  lead_time={lt}")
    return result
...
def main() -> None:
    ...
    print("-" * 70)
    print(f"{'fold':>4} {'test window':>22} {'ROC-AUC':>8} {'PR-AUC':>8} {'F1':>7} {'ECE':>7} "
          f"{'gap':>7} {'d':>6} {'recall':>7} {'prec':>6}")
    for r in results:
        if r["roc_auc"] is None:
            print(f"{r['fold']:>4} {r['test_start']}..{r['test_end']:>10}   (single-class window)")
            continue
        gap_disp = f"{r['auc_gap']:+.4f}" if r["auc_gap"] is not None else "n/a"
        print(f"{r['fold']:>4} {r['test_start']}..{r['test_end']:>10} "
              f"{r['roc_auc']:>8.4f} {r['pr_auc']:>8.4f} {r['f1']:>7.4f} {r['ece']:>7.4f} "
              f"{gap_disp:>7} {r['cohens_d']:>6.2f} {r['recall']:>7.3f} {r['precision']:>6.3f}")
    if valid:
        for key in ("roc_auc", "pr_auc", "f1", "ece", "cohens_d", "recall", "precision"):
            vals = np.array([r[key] for r in valid])
            print(f"  {key:10s}: mean={vals.mean():.4f}  std={vals.std():.4f}  "
                  f"min={vals.min():.4f}  max={vals.max():.4f}")
        gaps = np.array([r["auc_gap"] for r in valid if r["auc_gap"] is not None])
        if len(gaps):
            print(f"  {'auc_gap':10s}: mean={gaps.mean():+.4f}  std={gaps.std():.4f}  "
                  f"min={gaps.min():+.4f}  max={gaps.max():+.4f}  (n={len(gaps)}/{len(valid)} folds; "
                  f"large positive = train >> test = classic overfitting signature)")
        all_leads = [c["lead_time_days"] for r in valid for c in r["crises"]]
        captured = [d for d in all_leads if d is not None]
        if all_leads:
            print(f"  {'lead_time':10s}: {len(captured)}/{len(all_leads)} crises had an early "
                  f"crossing within {LEAD_TIME_LOOKBACK_DAYS}d" + (
                      f"; mean={np.mean(captured):.1f}d  median={np.median(captured):.1f}d  "
                      f"min={min(captured)}d  max={max(captured)}d" if captured else ""))
    print("=" * 70)

"""
run_minimal_model.py

Two branches from minimal_model.py, same walk-forward protocol as every
other run (6 folds, 5 seeds, per-seed scores, transition-AUC, save
after every fold):

    FF_lin   fixed physics, LINEAR encoder on all 124 fast features
    FF_vol   fixed physics, LINEAR encoder on the 40 volatility features

Both are compared to FF (0.8822) and LL (0.8849), already measured on
the same folds and seeds. Output is results/minimal_model.json, a
separate file; nothing already on disk is overwritten.

About two hours. If FF_vol holds, the model that survives is roughly
700 parameters with the physics fixed at six constants.

Place next to the 8 repo files, tilt_ablation.py, extra_metrics.py,
run_tilt_ablation.py, mechanism_ablation.py, minimal_model.py and this
file:
    python run_minimal_model.py
"""
...
def main() -> None:
        ...
        for mode in BRANCHES:
            ...
            for fold_id in range(1, len(FOLDS) + 1):
                ...
                if r.get("roc_auc") is not None:
                    ...
                    r["learned_scalars_seed0"] = __import__('mechanism_ablation').learned_scalars(m_probe)
                rows.append(r)
    ...
    print("\nReference, same folds, same seeds:")
    print("  LL  (31,381 params)  ROC-AUC 0.8849   PR-AUC 0.7444   seed sd 0.0284")
    print("  FF  (20,628 params)  ROC-AUC 0.8822   PR-AUC 0.7031   seed sd 0.0042")
    print("FF_lin ~ FF  -> the encoder nonlinearity is decorative.")
    print("FF_vol ~ FF  -> 40 volatility features and a linear map are the whole model.")
